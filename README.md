...
Файл состояния `data/tramite-state.json` (список уже учтённых шагов хронологии) на Railway **сбрасывается** при новом деплое, если не подключён том (после деплоя возможен лишний «первый» Telegram при режиме только при смене).
